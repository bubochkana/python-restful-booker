...
from src.models.bookings.booking_model import BookingModel, CreateBookingResponse
...
class TestBookingE2E:
    def test_booking_create_get_delete_e2e(self):
        ...
        comparison_results = CompareModel().compare_values(
            create_request_body.model_dump(), booking_model.booking.model_dump()
        )
        ...
        comparison_results = CompareModel().compare_values(
            create_request_body.model_dump(), get_booking_response.json()
        )
        assert_that(comparison_results, f"Following differences found: {comparison_results}").is_empty()
    ...
    def test_booking_create_update_e2e(self):
        ...
        comparison_results = CompareModel().compare_values(
            update_request_body.model_dump(), update_booking_model.model_dump()
        )
        assert_that(comparison_results, f"Following differences found: {comparison_results}").is_empty()
        ...
        comparison_results = CompareModel().compare_values(
            update_request_body.model_dump(), get_booking_response.json()
        )
        assert_that(comparison_results, f"Following differences found: {comparison_results}").is_empty()
