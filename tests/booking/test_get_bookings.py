...
from src.models import BookingId
...
class TestBookings:
    ...
    def test_filter_bookings_by_firstname_and_lastname(self, booking_client):
        response = booking_client.get_all_bookings(firstname="John", lastname="Smith")
        assert_that(response.json()).is_not_empty()

    def test_filter_bookings_by_checkin_and_checkout_dates(self, booking_client):
        response = booking_client.get_all_bookings(checkin="2018-01-01", checkout="2019-01-01")
        assert_that(response.json()).is_not_empty()
